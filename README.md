...
Structura HTML
CSS — Clase și variabile
Configurare și constante
Variabile globale
Gestionarea imaginilor per întrebare
Sistemul de răspunsuri DA/NU/N/A
Progres și validare formular
Upload imagini secțiunea 16
Modal completare incompletă
Generare PDF
Trimitere raport (trimiteRaport)
Construire HTML pentru PDF (buildPDFHtml)
PDF Feedback formular
Semnătură digitală
Inițializare formular (initForm)
Utilitare
Referință completă funcții
Jurnal modificări
---
...
Salvat ca HTML raw în GitHub (folder `rapoarte_raw/`)
...
const GAS_PROXY_URL = 'https://script.google.com/macros/s/.../exec'
...
Secțiuni text: adaugă în `TEXT_SECTIONS{}` + HTML corespunzător → se integrează în `textAnswers{}` și `getTotalQuestions()`
---
6. Variabile globale
```javascript
...
6. Injectează titlul: NR_Localitate_Data
...
PASUL 6 — Upload PDF feedback (secțiunea 19)
  ...
  POST GAS_PROXY_URL { folder: 'feedback-formular', fileExt: 'pdf', ... }
...
Compact DA — toate întrebările cu DA fără note/imagini → un singur rând verde: `"DA — întrebările: 1.1, 1.3, 1.7..."`
Compact N/A — similar, un singur rând gri: `"N/A — întrebările: 1.2, 1.5..."`
Detaliate — întrebările NU, sau DA/N/A cu note sau imagini atașate → afișate individual cu textul complet al întrebării
...
2. Inițializează textAnswers{}:
   Iterează TEXT_SECTIONS → textAnswers[key] = ''
...
GET GAS_PROXY_URL?action=nextNr
